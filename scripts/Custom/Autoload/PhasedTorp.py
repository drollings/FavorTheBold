...
"""
try:
	modPhasedTorp = __import__("Custom.Autoload.PhasedTorp")
	if(modPhasedTorp):
...
"""
# If you wish to specify a sub torp (the torpedo that get's fired after the initial torpedo has struck the shields)
# Then do this:
"""
try:
	modPhasedTorp = __import__("Custom.Autoload.PhasedTorp")
	if(modPhasedTorp):
		YourVariableName = modPhasedTorp.PhasedTorpedo(__name__, "PathOfSubTorp")
except:
	print "Phased Torpedo script not installed, or you are missing Foundation Tech"
"""
...
try:
	...
	class PhasedTorpedo(FoundationTech.TechDef):
		...
		def OnYield(self, pShip, pInstance, pEvent, pTorp):
			...
			if IsInList(pTorp.GetObjID(), self.lFired):
				pTorp.SetLifetime(0)
