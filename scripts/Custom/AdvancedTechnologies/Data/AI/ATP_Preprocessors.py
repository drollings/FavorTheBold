...
############################## Lennie Update:  Phalantium Firescript
# Notes to Programmers:
#
# i have duplicated Firescript in a new class that is NOT replaced
# at runtime with a streamlined version.  while this will be generally
# less efficient than the optimized code, it allows us to add/modify
# parameters and have them reflected in the AI.  the AI attack script
# itself must call the methods of this class, and not Firescript,
# for these changes to be articulated.
#
# for simplicity i have just copied/pasted Firescript into a new class
# in its entirety.  this particular Firescript derivative (no reference
# to inheritance) provides AI for using Phalantium-type torpedoes in 
# cases of extreme desperation.
#
# L Brown
# lbrown2@uiuc.edu
...
class PhFireScript:
	...
	######################################lennie update  
	# revised this method so that Phalantium Torps are possibly used as a last resort
	...
	def ChooseTorpType(self, pTorpSystem, vTargetLocation, fTargetSpeed):
		...
		iPhAmmoIndex = 2
		iIsPhalantium = 0
		import Tactical.Projectiles.Phalantium
		...
		for iAmmoIndex, pAmmo in lTorpTypes:
			...
			try:
				...
				# mark this torp as Phalantium
				if pScript.GetName() == Tactical.Projectiles.Phalantium.GetName():
					iIsPhalantium = 1
			except:
				fTorpDamage = 0.0
			...
			# exclude Phalantium torps from list and evaluation below
			if iIsPhalantium != 1:
				lTorpAmmoRatings.append(
					( iAmmoIndex, fIdealDistMin, fIdealDistMax, fIdealTargetSpeedMin, fIdealTargetSpeedMax, fRating ))
			else:
				# save the Phalantium Torpedo index for 
				# future reference and reset the boolean
				iPhAmmoIndex = iAmmoIndex
				iIsPhalantium = 0
		...
		# now evaluate ship status to determine if the situation is
		# sufficiently dangerous to warrant Phalantium Torpedo use
		pShip = pTorpSystem.GetParentShip()	

		# we'll define a period of desperation as any time when hull damage exceeds 60%
		# and always toggle back if it's above that level
		iCurrStatus = pShip.GetHull().GetCondition()
		iMaxCond = pShip.GetHull().GetMaxCondition() 

		iCurrTorp = pTorpSystem.GetAmmoTypeNumber()

		if iCurrStatus < (iMaxCond * 0.42):
			iSwitchPh = App.g_kSystemWrapper.GetRandomNumber(200)
			# 10% or 30% probability of changing torpedo loadout per 
			# update

			SwitchProbability = 179
			# ship nearly destroyed (integrity at 10%), so increase probability
			if iCurrStatus < (iMaxCond * 0.10):
				SwitchProbability = 139


			# after switching, keep using Phalantium as long as hull
			# sufficiently critical
			if pTorpSystem.GetNumAvailableTorpsToType(iPhAmmoIndex) > 0 and (iCurrTorp == iPhAmmoIndex or iSwitchPh > SwitchProbability):
				iChosenAmmo = iPhAmmoIndex
	
		# this prevents uncontrolled when toggling once we've switched to our last resort torps
		# by not allowing a switch back to standard munitions until hull is sufficiently repaired

		# Switch to the new ammo type, if it's not the current one.
		if (iCurrTorp != iChosenAmmo) and not ((iCurrTorp == iChosenAmmo) and (iCurrStatus < iMaxCond * 0.48)):
...
############################################################lennie update
#  Tries not to destroy a target in one hit by de-emphasizing
#  the targeting of critical subsystems, i.e. warpcore

class MultFireScript:
	# Arguments to the constructor:
	# pOurShip	- The ship object we're controlling
	# sTarget	- The object we're trying to shoot
	def __init__(self, sTarget, **kw):
		...
		##########################lennie update
		# default to targeting subsystems
		self.bChooseSubsystemTargets = 1
	...
	def Update(self, dEndTime):
		...
		# Cycle the iLastUpdate counter..
		iTargetSubsystem = App.g_kSystemWrapper.GetRandomNumber(200)

		###################lennie update
		# HACK: explicitly forcing more frequent subsystem targeting
		if iTargetSubsystem > 193:		
			self.iLastUpdate = -1
		else:
			self.iLastUpdate = ((self.iLastUpdate + 3) % (len( self.lWeapons ) + 2)) - 2
...
#		debug("Aimed weapon system has no direction info.")
	...
	def RateSubsystemForTargeting(self, pOurShip, pTargetShip, pSubsystem):
		...
		#######################lennie update
		# added various other systems to list for targeting
		...
		for eType, fRating in (
			...
			(App.CT_IMPULSE_ENGINE_SUBSYSTEM,	3.0),
			(App.CT_HULL_SUBSYSTEM,				-200.0),
			(App.CT_WARP_ENGINE_SUBSYSTEM,		3.0),
			(App.CT_POWER_SUBSYSTEM,	-100.0),
			):
			if pSubsystem.IsTypeOf(eType):
				fSystemRating = fRating
				break
		...
		fRating = 0.0
		#######################lennie udate
		# de-emphasize critical systems so that we don't 
		# so often destroy the target with one hit
...
############################################################lennie update
#  Multitargeting AI.  Frequently switches targets during a battle.  
#  Useful on ships with massive resources or strength.
#

class MultSelectTarget:
...
##
	...
	def SetRelativeImportance(self,
	  ...
	  fIsTarget = 0.0,	# Is it currently our target?
	  fShield = -0.2,	# Shield status
	  fWeapons = -0.2,	# Weapon status
	  fHull = -1.0,		# Hull status
...
#		debug("Updated %d AI's before endtime." % (iOriginalLength - len(self.lAIUpdateIDs)))
	...
	def GetTargetRating(self, pTarget):
		...
		if (pShip != None):
			# Shields...
			#############################lennie update
			# we want to decrease priority as damage increases

			pShieldSystem = pShip.GetShields()
			fShieldGood = 1.0 - pShieldSystem.GetShieldPercentage()
			...
			# Hull...
			iHullDamage = App.g_kSystemWrapper.GetRandomNumber(75)
	
			pHull = pShip.GetHull()	
			fHullGood = pHull.GetCombinedConditionPercentage() 

			# when hull gets to a random minimal level,
			# force a switch to another target
			if fHullGood < ((10 + iHullDamage) * 0.01):
				fHullGood = 30.0 
...
class AvoidObstacles:
	...
	def NeedToAvoid(self, pShip, fPersonalSpaceRadius, pObject):
		...
		pObjectShip=App.ShipClass_Cast(pObject)
		if pObjectShip:
			from Custom.AdvancedTechnologies.Data.Actions import ATP_ActionDecoder
			if ATP_ActionDecoder.IsActionShip(pObjectShip):
				return 0
...
class ATP_AvoidObstacles:
	def __init__(self):
		...
		## Modifications for Starbase
		self.IgnoreList = []
		self.FastStop = FALSE
		self.FastStopThreshold = 0.6
	...
	def AddObjectsToIngoreList(self,*kArgs):
		l =self.IgnoreList
		for pObj in kArgs:
			l.append(pObj)

	def SetPersonalSpace(self,f=2.5):
		self.fPersonalSpace = f

	def WithFastStop(self):
		self.FastStop = TRUE
	...
	def Update(self, dEndTime):
		...
		if self.vOverrideDirection:
			...
			if self.FastStop:
				pShip.SetImpulse(0.0, App.TGPoint3_GetModelForward(), App.ShipClass.DIRECTION_MODEL_SPACE)
			else:
				# Set the ship's course and speed.
				pShip.TurnTowardDirection( self.vOverrideDirection )
				pShip.SetImpulse( self.fOverrideSpeed, App.TGPoint3_GetModelForward(), App.ShipClass.DIRECTION_MODEL_SPACE)
	...
	def NeedToAvoid(self, pShip, fPersonalSpaceRadius, pObject):
		...
		if self.FastStop:
			V = pShip.GetWorldLocation()
			W = pObject.GetWorldLocation()
			W.Subtract(V)
			d = W.Length()
			W.Unitize()
			F = pShip.GetWorldForwardTG()
	
			if F.Dot(W) < self.FastStopThreshold:
				return 0
			#elif d < 7.5*km:
			#	return 1				

		OID = pObject.GetObjID()
		for pObj in self.IgnoreList:
			if pObj.GetObjID() == OID:
				return 0
