...
def CreateAI(pShip, *lpTargets, **dKeywords):
	...
	#A better way to assign AI...
	from Custom.AdvancedTechnologies.Data import QuickBattleAddon
	sAttackModule=pShip.GetShipProperty().GetAIString()
	...
	if sAttackModule=="FedAttack":
		if QuickBattleAddon.HasOffensiveTractors(pShip):
			sAttackModule="FedAttackWithTractors"
			pModule = __import__(sAttackModule)
			#print "ATP_BasicAttack: Assigned FedAttackWithTractors-AI for Ship ",pShip.GetName()
		else:
			pModule = __import__(sAttackModule)
			#print "ATP_BasicAttack: Assigned FedAttack-AI for Ship ",pShip.GetName()
	elif sAttackModule=="NonFedAttack":
		if QuickBattleAddon.HasOffensiveTractors(pShip):
			sAttackModule="NonFedAttackWithTractors"
			pModule = __import__(sAttackModule)
			#print "ATP_BasicAttack: Assigned NonFedAttackWithTractors-AI for Ship ",pShip.GetName()
		else:
			pModule = __import__(sAttackModule)
			#print "ATP_BasicAttack: Assigned NonFedAttack-AI for Ship ",pShip.GetName()
	elif sAttackModule=="CloakAttackWrapper":
		if QuickBattleAddon.HasOffensiveTractors(pShip):
			sAttackModule="CloakAttackWrapperWithTractors"
			pModule = __import__(sAttackModule)
			#print "ATP_BasicAttack: Assigned CloakAttackWithTractors-AI for Ship ",pShip.GetName()
		else:
			pModule = __import__(sAttackModule)
			#print "ATP_BasicAttack: Assigned CloakAttack-AI for Ship ",pShip.GetName()
	else:
		#print "ATP_BasicAttack: Assigned Normal-AI for Ship ",pShip.GetName()
		pModule = __import__("AI.Compound."+sAttackModule)
