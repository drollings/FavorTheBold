...
sMusicSfxFolder = 'scripts/Custom/AdvancedTechnologies/Data/Universe/Sfx'

lStandardMusic = [	("sfx/Music/EpisGen2.mp3", "Starting Ambient"),
...
lBattleMusic = [ ("Combat Panic", ("Cbt Panic 1",
				"Cbt Panic 2",
				"Cbt Panic 3",
				"Cbt Panic 4",
				"Cbt Panic 5",
...
class Music(ATP_EventHandlerObject):
	
	def __init__(self,ID=0):
		## Superclass
		ATP_EventHandlerObject.__init__(self,MUSIC_ID)

		## Own data
		self.AddHandler( GetPlayerShip().GetStateDoneEvent(WARPING) , 'NewSystem' )

		## Find the new music pieces
		import Foundation
		import string

		sFolder = sMusicSfxFolder + '/Music'
		lsFilenames = Foundation.GetFileNames(sFolder,'mp3')
		ltSound = []
		for sFile in lsFilenames:
			sShortfile = string.replace(sFile,'.mp3','')
			ltSound.append( ( sFolder+'/'+sFile , sShortfile ) )

		## Save it
		ltSound = lStandardMusic + ltSound

		## Reinitialise Dynamic Music
		pGame =  App.Game_GetCurrentGame()	
		DynamicMusic.Initialize(pGame,ltSound,(),lBattleMusic,DynamicMusic.StandardCombatMusic)			
		 

	def NewSystem(self,gEvent):
		## What system ?
		pSolar = GetPlayerShip().GetSolar()		

		## Entry tune
		sMusic = pSolar.GetEnterMusic()
		if sMusic:
			DynamicMusic.OverrideMusic(sMusic)
