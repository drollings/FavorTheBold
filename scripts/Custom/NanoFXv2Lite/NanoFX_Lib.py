...
def CreateRotationSeq(pShip, fRotation = None, fSpeed = None):
	...
	###
	pSequence.AddAction(App.TGScriptAction_Create("Custom.NanoFXv2Lite.NanoFX_ScriptActions", "SetNanoRotation", pShip, fRotation, fSpeed))
	###
	return pSequence
	###
...
def CreateFlickerSeq(pShip, fTotalTime, fFlickerSpeed = 0.2, sStatus = "On"):
	...
	if g_FlickerControl < CONST_MAXFLICKERS:
		...
		while (fFlickerTime < fTotalTime - 0.1):
			pSequence.AddAction(App.TGScriptAction_Create("Custom.NanoFXv2Lite.NanoFX_ScriptActions", "TurnOffLights", pShip), App.TGAction_CreateNull(), App.g_kSystemWrapper.GetRandomNumber(100) * 0.001 + fFlickerTime)
			pSequence.AddAction(App.TGScriptAction_Create("Custom.NanoFXv2Lite.NanoFX_ScriptActions", "TurnOnLights"), App.TGAction_CreateNull(), App.g_kSystemWrapper.GetRandomNumber(100) * 0.001 + fFlickerTime)
			fFlickerTime = fFlickerTime + fFlickerSpeed
		###
		if sStatus == "Off":
			pSequence.AddAction(App.TGScriptAction_Create("Custom.NanoFXv2Lite.NanoFX_ScriptActions", "TurnOffLights", pShip), App.TGAction_CreateNull(), fTotalTime)
			bFound = 0
		...
		if sStatus == "On":
			pSequence.AddAction(App.TGScriptAction_Create("Custom.NanoFXv2Lite.NanoFX_ScriptActions", "TurnOnLights"), App.TGAction_CreateNull(), fTotalTime)
			bFound = 0
			for sName in g_LightsOff:
				if pShip.GetName() == sName:
					bFound = 1
		...
		for kShip in pSet.GetClassObjectList(App.CT_DAMAGEABLE_OBJECT):
			for sName in g_LightsOff:
				if kShip.GetName() == sName or kShip.GetName() == None:
					pSequence.AddAction(App.TGScriptAction_Create("Custom.NanoFXv2Lite.NanoFX_ScriptActions", "TurnOffLights", kShip), App.TGAction_CreateNull(), fTotalTime + 0.5)
...
def CreateSpecialFXSeq(pShip, pEvent, sFXType, bPlay = 0):
	...
	if (sFXType == "PhalantiumFX"):
		import Custom.NanoFXv2Lite.SpecialFX.PhalantiumFX
		pSpecialFX = Custom.NanoFXv2Lite.SpecialFX.PhalantiumFX.CreatePhalantiumFX(pShip, pEvent)
	
	if (sFXType == "PlasmaFX"):
		import Custom.NanoFXv2Lite.SpecialFX.PlasmaFX
		pSpecialFX = Custom.NanoFXv2Lite.SpecialFX.PlasmaFX.CreatePlasmaFX(pShip, pEvent)
	
	if (sFXType == "Spatial"):
		import Custom.NanoFXv2Lite.SpecialFX.SpatialFX
		pSpecialFX = Custom.NanoFXv2Lite.SpecialFX.SpatialFX.CreateSpatialFX(pShip, pEvent)

	if (sFXType == "AtmosphereFX"):
		import Custom.NanoFXv2Lite.SpecialFX.AtmosphereFX
		pSpecialFX = Custom.NanoFXv2Lite.SpecialFX.SpatialFX.CreateAtmosphereFX(pSet, sName, sPlacement, fSize)
...
def CreateAtmosphereFX(pPlanet, sNifPath = "data/models/environment/planet.nif", sTexturePath = "Class-M"):
	...
	if Custom.NanoFXv2Lite.NanoFX_Config.sFX_Enabled == 1:
		import Custom.NanoFXv2Lite.SpecialFX.AtmosphereFX
		Custom.NanoFXv2Lite.SpecialFX.AtmosphereFX.CreateAtmosphereFX(pPlanet, sNifPath, sTexturePath)
