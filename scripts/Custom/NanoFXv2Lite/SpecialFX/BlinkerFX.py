...
def CreateBlinkerFX(pShip):
	...
	sFile = "Custom/NanoFXv2/SpecialFX/Gfx/Blinker/Blinker.tga"
	### Setup for Effect ###
	...
	for iBlinkers in range(len(pBlinkers)):
		...
		pLight = Custom.NanoFXv2Lite.NanoFX_ScriptActions.CreateControllerFX(sFile, pEmitFrom, pAttachTo, fSize * 1.1, vEmitPos, fFrequency = fPeriod, fLifeTime = fLifeTime, iTiming = fDuration, sType = sType, fRed = 255.0 * pCol.GetR(), fGreen = 255.0 * pCol.GetG(), fBlue = 255.0 * pCol.GetB())
		pSequence.AddAction(pLight)
		pLight = Custom.NanoFXv2Lite.NanoFX_ScriptActions.CreateControllerFX(sFile, pEmitFrom, pAttachTo, fSize * 0.5, vEmitPos, fFrequency = fPeriod, fLifeTime = fLifeTime, iTiming = fDuration, sType = sType)
		pSequence.AddAction(pLight)
