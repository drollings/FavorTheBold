...
import SpeciesToShip

SpeciesToShip.Initialize()

class MPTextButtonFactory(FoundationMenu.TextButtonFactory):
	...
	def MakeIntButton(self, eType, iSubType, uiHandler, fWidth = 0.0, fHeight = 0.0):
		...
		if self.isTGL:
			pButton = App.STButton_CreateW(MissionShared.g_pShipDatabase.GetString (SpeciesToShip.GetScriptFromSpecies ('ships.' + self.text)), pEvent)
		elif self.text is not None:
			pButton = App.STButton_Create(self.text, pEvent)
		else:
			pButton = App.STButton_Create('?!?', pEvent, fWidth, fHeight)

		if (iSubType == g_iSpecies):
			pButton.SetChosen (1)
			global g_pChosenSpecies
			g_pChosenSpecies = pButton
		pEvent.SetSource(pButton)
		# print pEvent, pButton, pEvent.GetSource()

		return pButton


class MPShipMenuBuilderDef(FoundationMenu.ShipMenuBuilderDef):
	def __init__(self, tglDatabase):
		self.tglDatabase = tglDatabase
		self.textButton = MPTextButtonFactory(tglDatabase)
	...
	def __call__(self, raceShipList, menu, buttonType, uiHandler, fWidth = 0.0, fHeight = 0.0):
		...
		for race in raceList:
			...
			for key in shipList:
				...
				if ship.__dict__.has_key('iMPSpecies'):
					self.textButton(ship.name)
					num = ship.iMPSpecies
					# print ship, num
					pRaceButton.AddChild(self.textButton.MakeIntButton(buttonType, num, uiHandler, fWidth, fHeight))
...
def BuildShipSelectWindow(pMissionPane, pDatabase):
    ...
    import Multiplayer.SpeciesToShip
    import Foundation
    mpGameMode = Foundation.pCurrentMode

	# Create the window
    pWindow = App.STStylizedWindow_CreateW('StylizedWindow', 'NoMinimize', pDatabase.GetString('Select Your Ship'), 0.0, 0.0, None, 1, SHIPS_WINDOW_WIDTH, SHIPS_WINDOW_HEIGHT)
	...
	#########################################
	# Create the buttons
    if (not shipMenuBuilder):
        import FoundationMenu
        global shipMenuBuilder
        shipMenuBuilder = MPShipMenuBuilderDef(pDatabase)

    shipMenuBuilder(mpGameMode.playerShipMenu, pSubPane, ET_SELECT_SHIP_SPECIES, pMissionPane)
    pSubPane.ResizeToContents()
    return pWindow


def _BuildShipSelectWindow (pMissionPane, pDatabase):
	import MissionShared
	import Multiplayer.SpeciesToShip
	# Create the window
...
def BuildRegionSelectWindow (pMissionPane, pDatabase):
	...
	for iIndex in range(1, Multiplayer.SpeciesToSystem.MAX_SYSTEMS):
		...
		# Create the button.
		# Dasher42's changes: a quick and dirty way to support 
		sText = Multiplayer.SpeciesToSystem.GetScriptFromSpecies (iIndex)
		if MissionShared.g_pSystemDatabase.HasString(sText):
			pButton = App.STButton_CreateW(MissionShared.g_pSystemDatabase.GetString(sText), pEvent)
		else:
			pButton = App.STButton_Create(sText, pEvent)
...
def SelectSpecies (iSpecies):
	...
	sText = Multiplayer.SpeciesToShip.GetSideFromSpecies (g_iSpecies)
	print sText
	if MissionShared.g_pDatabase.HasString(sText):
		g_pShipSideText.SetTextW(MissionShared.g_pDatabase.GetString(sText))
	else:
		g_pShipSideText.SetText(sText)
	...
	sText = Multiplayer.SpeciesToShip.GetScriptFromSpecies (g_iSpecies)
	if MissionShared.g_pShipDatabase.HasString(sText):
		g_pShipSideText.SetTextW(MissionShared.g_pShipDatabase.GetString(sText))
	else:
		g_pShipSideText.SetText(sText)
	...
	sText = Multiplayer.SpeciesToShip.GetScriptFromSpecies (g_iSpecies)
	if MissionShared.g_pShipDatabase.HasString(sText + " Description"):
 		pText.SetStringW(MissionShared.g_pShipDatabase.GetString(sText + " Description"))
	else:
		try:
			pModule = __import__(sText)
			sText = pModule.sDescription
 			pText.SetString(sText)
 		except:
 			pText.SetString('Missing description')
...
def SelectSystem(iSystem):
	...
	if (g_pSystemDescWindow):
		...
		sText = Multiplayer.SpeciesToSystem.GetScriptFromSpecies (g_iSystem)
		if MissionShared.g_pSystemDatabase.HasString(sText + " Description"):
 			pText.SetStringW(MissionShared.g_pShipDatabase.GetString(sText + " Description"))
		else:
 			pText.SetString(sText)
