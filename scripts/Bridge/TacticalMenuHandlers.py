	...
	if MissionLib.GetPlayerShipController() in ( None, "Tactical" ):
		...
		if bAcknowledge and pTactical:
			...
			if pPlayer and (not pPlayer.GetAI()):
				...
				pPulses = pPlayer.GetPulseWeaponSystem()
				bPulsesOn = 0
				if (pPulses):
					bPulsesOn = pPulses.IsOn()

				# print pTorps, bTorpsOn
				# print pPhasers, bPhasersOn
				...
				if bAcknowledge == 2:
					...
					# KILL!
						...
						if (not bTorpsOn and not bPhasersOn and not bPulsesOn):
							g_iOrderState = -1
							pSequence.AppendAction(App.CharacterAction_Create(pTactical,
											App.CharacterAction.AT_SAY_LINE, "NeedPower",
											None, 1, pDatabase, App.CSP_SPONTANEOUS))
							# Dasher42's change:  fixed a bug in which the "NeedPower" line wasn't
							# being spoken because the UpdateOrders call superceded it with the generic
							# response.
							UpdateOrders(0)
						else:
