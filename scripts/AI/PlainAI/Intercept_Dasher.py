...
class Intercept_Dasher(BaseAI.BaseAI):
	...
	fMaxPredictionDistance	= 10000.0
...
#			debug("Intercept update done: no set")
		...
		if (pObject != None):
			...
			if fMaxSpeed > 0:
				...
				if (pPhysicsObject == None) or fCurrentSpeed < 20.0: # EXPERIMENTAL CHANGE
				...
				fRealDistance = fObjectDistance - (pObject.GetRadius() * 1.2)
				pPlanet = App.Planet_Cast(pObject)
				if pPlanet:
					fRealDistance = fRealDistance - (pPlanet.GetAtmosphereRadius() * 3.0)
					fDistance = fRealDistance

				# If we're extremely far away, override the normal
				# speed limitations and do an in-set warp.
				bWarping = 0
				if fDistance > 375.0:
					if fAngle < 0.2 or fCurrentSpeed > 20.0:
						bWarping = 1	# We need to slow down and turn
				elif fCurrentSpeed > 20.0:
					bWarping = 1	# We need to slow down FAST

				if bWarping:	AI.FoundationAI.EnableInterceptSpeed(pShip)
				else:			AI.FoundationAI.DisableInterceptSpeed(pShip)

				# print bWarping, 'Distance', fRealDistance, 'Angle', fAngle, 'Speed', fCurrentSpeed

				fSpeed = fMaxSpeed

				if bWarping:
					if fAngle < 0.2:
						fStopDist = AI.FoundationAI.DecelerationDistance(fCurrentSpeed, fMaxAccel * 0.35)
						fRatio = fDistance / fStopDist
						a = AI.FoundationAI.accelBoost
						# Hopefully this will adapt more readily to what Apollo is doing. -Dasher42
						stopratio = 1.0 + ((a * a) / min(a * 60, a * a) * 0.2)

						# print 'Warping', fCurrentSpeed, fStopDist

						if fRatio < stopratio:
							fSpeed = 0 # -2.0 / 9.0
						else:
							fSpeed = fCurrentSpeed * fRatio
					else:
						fSpeed = 0


				else:
					fStopDist = fMaxSpeed * fMaxSpeed / (fMaxAccel * 1.8)
					if fDistance < fStopDist:
						# We need to stop.  Make sure our velocity
						# is low enough that we can stop in time.
						fMaxVel = math.sqrt(max(fMaxSpeed * fMaxSpeed - (2 * fMaxAccel * fDistance), 1.0))
				...
				fSpeed = max(min(fSpeed, self.fMaximumSpeed), -2.0 / 9.0)
