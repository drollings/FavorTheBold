...
class ATP_FollowWaypointsBis(BaseAI.BaseAI):
	...
	def SetTargetWaypointName(self, pcWaypoint,pcWaypointLast=None): #AISetup
		self.pcTargetWaypoint  = pcWaypoint
		self.sOriginalWaypoint = pcWaypoint
		self.pcWaypointLast    = pcWaypointLast

	def SetPrecision(self,fCloseEnough=8.0):
		self.fCloseEnough = fCloseEnough
	...
	def Update(self):
		...
		if (pObject != None):
			...
			if fDistance < self.fCloseEnough:
				...
				if (pWaypoint != None and self.pcWaypointLast != self.pcTargetWaypoint):
